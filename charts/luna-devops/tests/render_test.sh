...
assert_contains "$default_render" 'requirepass "\$password"' 'the built-in Redis server does not require authentication'
assert_not_contains "$default_render" '^[[:space:]]+- name: REDIS_(USERNAME|PASSWORD|DB)$' 'split Redis environment variables are still rendered'
