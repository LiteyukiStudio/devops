...
```bash
cp .env.production.example .env
# 修改 .env 中的 SECRET_ENCRYPTION_KEY、BOOTSTRAP_TOKEN 和 REDIS_ADDR
docker compose up -d
```

完整平台默认使用 `APP_ENV=production`，不会创建或展示固定开发管理员。`SECRET_ENCRYPTION_KEY`、首次初始化使用的 `BOOTSTRAP_TOKEN` 和包含凭据的 Redis URI 必须在启动前设置；完成首个管理员初始化后，应轮换或移除一次性的 `BOOTSTRAP_TOKEN`。
...
完整平台 compose 内置 PostgreSQL / Redis 只在容器网络内访问，不占用宿主机 `5432` / `6379`，避免和开发依赖冲突；Redis 默认要求密码，API、Worker 和任务工具只读取完整的 `REDIS_ADDR` URI，例如 `redis://default:password@redis:6379/0`。API 完成数据库 migration 且 `/healthz` 通过后，Compose 才启动 Worker，避免空库初始化期间提前访问业务表。对外只暴露 API 的 `8088`。API 镜像内嵌前端 SPA 静态文件，根路径和前端路由由后端 fallback 到 `index.html`，`/api/*` 和 `/healthz` 继续走后端接口。
