...
var localizedMessages = map[string]map[string]string{
	"zh-CN": {
		...
		"git.repository_not_found":         "Git 仓库不存在，或当前凭据无权访问该仓库。请检查仓库绑定和 Git 凭据。",
	...
	"en-US": {
		...
		"git.repository_not_found":         "The Git repository does not exist, or the current credential cannot access it. Check the repository binding and Git credential.",
