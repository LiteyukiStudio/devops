...
- **MUST 品牌命名边界**：涉及用户可见品牌、产品标题、文档标题、OpenAPI/Swagger 展示标题、通知模板、示例界面和营销/说明文案时，统一使用 `Luna DevOps`。涉及技术标识、兼容性标识、仓库/镜像/chart/package/module 名、Kubernetes label/annotation/domain、数据库名、配置 key、metrics prefix 和历史迁移时，继续沿用既有 `liteyuki`、`liteyuki-devops` 或 `LiteyukiStudio/devops`，不要为了品牌统一而改动。
