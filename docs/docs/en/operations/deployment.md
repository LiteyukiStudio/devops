...
The project-space Web Console master switch is enabled when a project space is created. Project Owners and Admins can disable it for the whole space; no deployment target can re-enable Web Console while that switch is off. A deployment target can only inherit the project setting or further disable itself, so the effective policy is “project enabled and target not disabled.” This switch only decides whether the deployment target allows terminal access. It does not relax roles or MFA: the application release console remains limited to project Owners, Admins, and Developers, while Pod terminals on the cluster resource page remain platform-administrator only.
...
Before opening an interactive terminal from an application release, the frontend calls the normal HTTP terminal authorize preflight. The preflight checks project role, project/deployment-target state, the effective Web Console switch, and the MFA assertion, so `mfa_required` can be handled by the shared dialog and retried after verification. Only after a 204 does the frontend open the WebSocket. The WebSocket repeats the checks before upgrading and every three seconds while connected. Only real terminal input refreshes the idle deadline, with writes throttled; resize, ping, and polling do not renew it, and the absolute deadline never moves. Signing out, membership or role changes, policy changes, resource deletion, or assertion expiry closes the shell; preflight success is not a reusable authorization ticket.

Deployment-target data export requires an interactive cookie-authenticated session and project Owner or Admin role. A personal access token is rejected with `auth.interactive_session_required` even if it has a data-export scope. Before opening the archive download, the console calls a normal HTTP authorize endpoint so the shared MFA dialog can handle the `data_export` purpose. It issues a 60-second one-time ticket bound to the current user, session, project, application, and deployment target. The download atomically consumes that ticket and repeats the role, resource, and MFA checks. Production replicas store only the ticket hash in shared Redis and fail closed when Redis is unavailable. Each export uses an isolated temporary Pod and cleans up only that Pod, so concurrent exports cannot interrupt one another. Exports include platform-managed or existing PVCs; `emptyDir` is never exported.
