...
- `DataList` 的灰色表头保持为无边框连续表面：不绘制表头底线或固定操作列的表头分隔线；正文固定列仍保留必要的结构边界。
...
- `DataList` 外壳不绘制外边框，以圆角和表面底色区分背景；行分隔、工具区与分页区边界仍保留，避免信息结构变弱。
- 列表页的搜索、筛选和排序控件统一放入 `DataList` 表头工具区；创建等页面级主操作放在 `PageChrome` 标题右侧。不要在列表外再增加独立查询工具栏。`DataList.toolbar` 用于搜索框之外的筛选和排序组合。
