...
第一版内置 Node.js 服务、Node.js 静态站点、Python + uv、Go、Rust 和纯静态站点模板。选择模板后，可以调整运行时版本、依赖安装命令、构建命令、启动命令和服务端口等必要参数，并在保存前预览生成结果。
...
平台只根据 `package.json`、`pyproject.toml`、`go.mod`、`Cargo.toml`、`index.html` 等文件提供建议，不会替用户猜测启动命令。实际命令仍应以项目文档为准。
