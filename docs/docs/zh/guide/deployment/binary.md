...
二进制部署适合调试、离线排障或特殊环境验证，不作为推荐部署方式。正式使用请优先选择 [Kubernetes (Helm) 部署](/guide/deployment/kubernetes-helm) 或 [Docker Compose 部署](/guide/deployment/docker-compose)。

原因很简单：二进制部署需要你自己维护 PostgreSQL、Redis、进程守护、日志、升级和前端静态资源。容器化部署会少很多重复工作。
