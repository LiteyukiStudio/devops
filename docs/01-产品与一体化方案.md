...
- GitAccount 保存加密后的 `accessTokenRef`、`refreshTokenRef` 等密钥引用，不保存明文 token。
- 平台 API 支持仓库列表、分支列表、文件读取，前端可用同一接口检测 Dockerfile 和 `.devops/app.yaml`。
...
MVP 实现中使用 `RepositoryBinding` 承载应用与仓库关系：每个 repository 来源应用可以绑定一个 GitProvider、一个当前用户 GitAccount、owner/repo、cloneUrl、defaultBranch、webhookStatus、webhookId、webhookSecret、lastEvent、lastCommitSha、lastWebhookAt 和 credentialRef。`webhookStatus` 初始为 `pending`，创建成功后更新为 `created`，创建失败后更新为 `failed`。
...
- Registry 凭据测试通过受控 `/v2/` ping 完成，主要验证 endpoint 连通和 Basic Auth 可用性；Harbor 项目、机器人账号和漏洞扫描 API 后续再接入适配器。
