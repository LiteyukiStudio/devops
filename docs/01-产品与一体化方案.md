...
进入应用构建页面
  -> 选择平台构建模板
  -> 选择应用和本次构建分支
  -> 按该分支探测 Dockerfile 和 context 候选
  -> 选择镜像推送目标
  -> 保存构建配置
```
...
- Dockerfile 和 context 候选按构建表单选择的分支读取仓库结构，但用户仍可手动填写路径。
...
- 开发模式下后端未显式设置 `ENV_FILE` 时默认尝试读取 `.env.dev`；也可以通过 `ENV_FILE` 指定其它 `.env.*` 文件。启动时可以打印环境变量文件加载状态和文件路径，便于确认本地配置是否生效；生产模式不打印环境变量文件路径。
...
- Dockerfile 路径和构建上下文使用“可输入 + 候选建议”交互，探测结果作为 datalist 建议，允许用户手动修正特殊路径，避免探测漏报时无法保存。
...
- 普通用户列表只展示可用镜像站摘要，不展示管理员维护的 endpoint、namespace、capabilities 等配置字段；资源维护者和平台管理员可以看到完整配置。
...
RuntimeCluster
  ...
  - scope: global | project | user
  - ownerRef
  - kubeconfig: write-only YAML input
  - credentialSet
  - defaultNamespace
  - defaultIngressClass
  - defaultDomainSuffix
...
- 普通用户列表只展示可用集群的名称、类型、作用域、默认状态和健康状态，不展示管理员维护的 endpoint 或 kubeconfig 配置。
...
5. 平台将 BuildRun / BuildJob 放入数据库构建队列
6. Builder Agent 主动轮询平台 API 并领取任务
...
11. Builder Agent 实时回传日志，并在结束时回传状态、镜像 tag 和 digest
12. 镜像进入可发布状态
```
...
flowchart TD
    ...
    Queue --> Scheduler["Scheduler / DB Queue"]
    Builder["Builder Agent"] -->|heartbeat / claim / realtime logs / result| API
    API --> Scheduler
...
- Builder Agent 通过平台 API 长轮询或短轮询领取任务，不直接连接 Redis。
- Builder Agent 可以运行在 Docker Compose、VM、裸机或 Kubernetes 中；构建执行不依赖 Kubernetes Job。
- executor 容器只负责单次构建执行，不持有平台 API 权限，不直接回调 API。
- executor 默认只访问 Git、Registry、包管理源和平台允许的构建出口；平台 API 仅由 Builder Agent 访问。
- 构建日志由 Builder Agent 从 executor stdout/stderr 实时采集，按时间或大小批量追加到平台 BuildLog；构建结果由 Builder Agent 从约定 `result.json` 读取，再通过 API 回写。
...
- Dockerfile 和 context 使用本次构建表单选择的值；候选项来自该分支的仓库结构探测，应用默认值只作为兜底。
- 使用 BuildKit rootless 构建镜像。
- 推送镜像到工作流编辑时选择的镜像站。
...
builder-agent
  - 主动向 API heartbeat
  - 主动通过 API claim 构建任务
  - 读取 BuildRun、Application、RepositoryBinding、RegistryCredential
  - 申请并发额度
  - 每次构建启动独立 executor 容器
  - 收集日志、状态、digest 和镜像引用
  - 通过 API 回写 BuildRun、BuildJob、ContainerImage
...
- Builder Agent 使用 builder token 访问 heartbeat、claim、logs、complete/fail API。
- Builder Agent 通过 `builderId` 和 `leaseUntil` 认领任务，lease 过期后任务可被重新领取。
...
- 右侧内容区顶部固定展示当前页面标题和一行说明，标题来源于当前路由或导航项，不再展示站点名称。
...
- kubeconfig、JSON 配置、YAML 清单、脚本片段等长文本代码输入统一使用公共 `CodeEditor` 组件，提供等宽字体、语言标签、行号和语法高亮；普通描述和备注才使用 `Textarea`。
...
- Git/OIDC OAuth state 运行时表名必须与 migrations 表名一致，统一使用 `git_oauth_states` 和 `oidc_auth_states`。
...
- 生产模式必须配置 `SECRET_ENCRYPTION_KEY`，并设置 session cookie `Secure=true`。
...
- Git、OIDC 和 Registry 等后端出站请求必须统一使用 SSRF/egress 组件；普通用户策略默认禁止私网和特殊地址，平台管理员策略允许受控内网集成，管理员可在安全设置中维护域名/IP 黑名单、特许白名单和端口规则。
