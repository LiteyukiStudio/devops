...
- 用户可以配置平台构建模板。
- 平台可以通过 webhook 创建 BuildRun，并由 Builder Agent 领取 BuildJob。
- 用户可以触发构建并看到构建状态。
- 用户可以在模块配置中选择镜像推送目标。
- 平台可以读取镜像版本。
...
- 代码仓库应用可以绑定仓库。
- 镜像应用不强制绑定仓库。
- 应用信息只维护名称、标识和图标等基础展示字段；仓库绑定、Dockerfile 路径、构建上下文、目标镜像、构建策略、变量和密钥和部署/访问入口由模块配置维护。
...
- 项目 Hook 页面只作为构建/部署脚本库：Hook 本身不维护启停状态，也不表达构建执行顺序。模块配置负责选择本模块要运行的 `preBuild` / `postBuild` Hook，并在模块内保存拖拽排序；部署 Hook 后续按部署配置或发布流程绑定，不复用项目 Hook 页排序。
...
- 模块配置表单提供构建默认值联动：选择 Dockerfile 后默认将构建上下文和构建目录设为 Dockerfile 所在目录；目标镜像引用模板在用户未手动编辑前按镜像站命名空间、应用标识和模块标识生成，用户手动修改后停止自动覆盖。
- 项目空间概览定位为项目级 dashboard，只展示项目下应用数量、最近构建、发布健康、访问入口、变量密钥和成员摘要；单应用模块、部署配置和访问明细仍在应用详情内维护，避免定位重复。
...
- 应用信息不保存默认构建入口；真正构建时以 BuildRun 选择的 `moduleId` 和 `sourceBranch` 为准。
- BuildRun 的 Dockerfile 和 context 必须继承选中的应用模块配置；如需修改，用户必须回到应用配置页的模块配置区域调整，后端也忽略构建触发请求中传入的 Dockerfile/context。
...
- 创建应用和编辑应用只维护基础展示字段，不在弹窗中选择来源类型、仓库、镜像或构建参数；代码仓库、Dockerfile、构建上下文、目标镜像、部署策略、Webhook 和访问由应用详情的模块配置、部署配置和访问配置维护。应用详情概览采用看板式摘要，展示模块状况、最近构建、部署健康和访问入口。
- 项目空间应用列表只展示应用基础摘要和操作入口，不展示来源类型、仓库来源或服务端口，避免和模块配置职责重复。
- 按状态筛选。
- 进入应用运行详情或构建部署详情时才使用独立页面；基础配置编辑不作为单独页面入口。
- 应用列表必须提供明确的“进入详情”按钮，进入后通过应用详情页 `ContentTabs` 管理概览、模块、构建、部署和访问。
...
- Webhook 生命周期归属于应用的 RepositoryBinding：外部 Git 平台只负责把仓库事件推送到平台；平台收到事件并校验签名后，再按应用下的模块配置触发规则决定创建哪些 BuildRun、是否继续自动部署。模块配置不直接拥有外部 Git webhook。
...
- BuildProvider 配置同时维护稳定唯一标识和友好显示名称：唯一标识用于平台内部引用、API 参数和审计定位，显示名称用于前端列表、表单与选择器的人类可读展示；前端展示构建器配置和构建器选择时必须同时呈现这两个值。
...
- 自动配置 Webhook，并支持应用配置页和仓库绑定列表重新配置。
- 读取分支、提交、标签。
- 生成或维护平台模块配置。
- 将仓库与应用建立关联。
...
- Dockerfile 路径和构建上下文在应用配置页的模块配置区域维护；构建触发时只读继承选中的模块配置，不再允许按 BuildRun 临时修改，避免同一模块配置在不同运行中漂移。
...
- 默认只提供查看、刷新、定位到业务对象等安全操作；重启、扩缩容、删除等会改变集群状态的操作后续必须通过平台业务动作和审计流转，不直接裸操作任意 K8s 对象。
...
- 为避免 Kubernetes 资源名过长，Kubernetes Namespace、Deployment、Service、ConfigMap、Secret 等运行态资源统一由平台内部 ID 派生，不再由用户可读名称生成。Project slug、Application slug、Environment slug 仍保留长度防呆，用于平台路径、默认域名和未来可读场景；DeploymentTarget `name` 仅作为平台展示名，允许中文、空格和符号。
...
6. Builder Agent 通过 HTTP 轮询平台 API，携带自身标签和 scope 抢占式领取匹配任务
...
11. Builder Agent 通过 HTTP 实时回传日志，并在结束时回传状态、镜像 tag 和 digest
12. 镜像进入可发布状态
```
...
flowchart TD
    ...
    Builder["Builder Agent"] -->|heartbeat / claim / logs / progress / result| API
...
- API Server 只负责权限校验、BuildRun/BuildJob 创建、任务分配和状态落库，不直接执行构建。
- Builder Agent 仅通过 HTTP 领取任务和回写状态，使用平台配置的 Builder token 鉴权。
...
- 构建日志由 Builder Agent 从 executor stdout/stderr 实时采集，按时间或大小批量写入平台 API；构建结果由 Builder Agent 从约定 `result.json` 读取，再通过 API 落库。
...
- Dockerfile 和 context 使用 `ApplicationModule` 写入 BuildRun 的值；构建触发请求中即使传入对应字段，API 也会用选中的模块配置覆盖。
- 使用 BuildKit rootless 构建镜像。
- 推送镜像到模块配置选择的镜像站。
...
- 平台必须校验 webhook 签名。
- 平台只接受已绑定仓库的事件。
- Webhook 只作为应用仓库事件入口；Module 只保存事件、分支、tag、手动/API 等触发匹配规则和后续动作配置。
...
- 目标镜像引用模板保存到 `ApplicationModule`，后续构建和部署默认复用本次选择的模块配置。
- 镜像站只维护 endpoint、类型、作用域和能力；`team/app` 这类 repository 路径属于模块配置，不放在镜像站配置里，也不作为一次性 BuildRun 配置漂移。
...
- Release 创建和回滚请求会投递 `deploy:run` 任务；Worker 固定按 `project-{projectSlug}` 幂等创建和使用 Project namespace。
...
- 应用支持多个 `ApplicationModule`。每个配置独立维护构建名称、稳定标识、仓库绑定、Dockerfile、构建上下文、目标镜像、目标镜像站、构建变量和密钥、Builder 标签和同配置并行策略；`BuildRun.moduleId` 记录本次构建使用的配置。默认同一模块配置排队，不同模块配置可并行；当配置为并行时，同一模块配置也允许多个任务同时被 Builder 领取。应用配置页不再维护服务端口和单一仓库，服务端口随模块配置内的部署/访问入口配置。
- BuildKit registry 镜像层缓存由 Builder 环境变量统一控制。启用 `BUILDER_CACHE_ENABLED=true` 后，Builder 默认把缓存推送到目标镜像同仓库的 `:buildcache` tag，可通过 `BUILDER_CACHE_TAG` 调整；executor 在 `buildctl` 中追加 `--import-cache type=registry` 和 `--export-cache type=registry,mode=max`，用于跨一次性 executor 和跨 Builder 复用镜像层缓存。
- 模块配置表单直接选择 Git 账号和仓库，平台复用或创建应用下的 RepositoryBinding；Dockerfile、构建上下文和构建路径按所选仓库自动探测，构建提供者不再作为用户可选项展示，由 Builder HTTP 调度自动匹配能力标签。表单按工作流分为基础信息、代码来源、构建产物、触发与调度、部署配置和访问入口分区，弹窗正文内部滚动并固定底部保存操作，避免长表单把主要动作挤出视野。
- 应用配置页采用左主右辅布局：左侧维护应用基础信息和模块配置，右侧仅展示目标镜像模板变量参考。项目空间级变量和密钥在项目工作台集中维护，并默认注入该项目空间内所有构建容器；普通变量可回显编辑，密钥只保存后端引用且不会再次回显，编辑时只能输入新值覆盖。
- 应用部署链路由 `DeploymentTarget` 表达，绑定 `moduleId -> environmentId`，配置自动部署和是否需要人工确认；branch/tag 匹配归属于 `ApplicationModule`，因为它决定这条构建到部署链路响应哪些 Git 事件。构建成功落库后，API 先按 Module 的 branch/tag pattern 判断，再按启用且无需人工确认的 DeploymentTarget 自动创建 Release 并投递 `deploy:run`，使构建和部署链路化，同时保留 Release 审计、回滚和环境隔离。
...
- GatewayRoute 当前尚未持有 DeploymentTarget 引用；在一个应用同环境存在多个部署配置时，需要补充网关绑定部署目标字段后再让 Ingress 指向 `{applicationSlug}-{deploymentSlug}-{environmentSlug}` Service。
...
api-server
  ...
  - 维护数据库构建队列，并按 Builder 上报标签和 scope 分配任务
...
- Builder Agent 仅支持 HTTP transport，通过 `BUILDER_API_URL` 连接平台 API，并使用 `BUILDER_TOKEN` 鉴权。
- Builder Agent 每次 claim 都上报 `agentId`、名称、scope、能力标签、executor 和并发信息。API 从数据库队列中使用行级锁选择 queued 或 lease 过期的 running BuildJob；应用构建标签必须被 Builder 标签全部命中，scope 为空或包含 `global` 时对所有构建可用，也可配置 `project:<projectId>`、`user:<userId>`。同一 `ApplicationModule` 默认只允许一个 running BuildRun，claim 时会锁定模块配置行并检查仍持有 lease 的 running job；配置为 `parallel` 时跳过该限制。
- 多个 Builder 同时轮询时通过数据库 `FOR UPDATE SKIP LOCKED` 抢占式领取任务，避免同一个任务被重复分配。
...
- Builder Agent 通过 `builderId`、`leaseToken` 和 `leaseUntil` 认领任务；日志、进度、续租、完成和失败回调都必须携带当前 `leaseToken`。Worker 定时扫描运行中 BuildJob，lease 过期后将 BuildRun/BuildJob 标记为 `lost`，不自动重新领取，避免同一个构建被重复执行；迟到的旧回调不能覆盖 `canceled`、`lost` 或其他终态。
- Builder Agent 每次构建启动独立 executor 容器。
- BuildRun 支持从历史运行重试，重试会复制原运行的分支、Tag、commit、镜像目标、变量和密钥和 provider，创建新的 queued BuildRun / BuildJob。
- queued/running 状态的 BuildRun 支持用户主动终止；平台将 BuildRun/BuildJob 标记为 `canceled`，Builder 轮询取消状态后停止已经启动的 Docker executor，后续迟到的 complete/fail 回调不得覆盖取消状态。
...
- Builder 心跳、构建运行创建时间以及后续任务类更新时间复用统一时间格式化工具：10 小时内显示相对时间，超过后显示 `YYYY/MM/DD HH:mm`；任务耗时统一通过公共耗时格式化函数展示。
- 应用构建运行列表必须使用后端分页，支持搜索以及 Event、Status、Branch、Actor 筛选；分页组件采用统一的 GitHub 风格页码、Previous 和 Next 交互。
- 构建运行行采用紧凑元数据：标题展示触发方式和模块配置名称，例如 `手动触发构建：前端`；代码来源 badge 展示 `branch owner/repo #buildId 由 platformUser 触发 · gitAuthor 提交 commit`；第二个 badge 在未成功时展示实时状态或当前步骤，构建成功且存在镜像时才展示镜像图标和 `imageRef`。构建 ID 保留 `#` 前缀，提交 hash 不加 `#`。平台触发人与 Git 提交者必须分开展示，不能用平台用户冒充 Git 作者。
...
- Builder progress 由后端写入原始枚举 key，例如 `clone_repository`、`load_dockerfile`、`run_command`、`push_image_layers`，前端按 key 做 i18n 展示；后端不返回面向用户的本地化文案。构建行右侧时间区域只展示时间、耗时和任务摘要，不展示当前阶段。BuildKit build 阶段使用 `--progress=plain` 保留可读实时日志，Builder Agent 从 plain 日志中解析阶段并通过 Builder HTTP progress 接口旁路上报，不能用 rawjson 替代普通日志展示。
- 构建列表和部署列表状态刷新使用统一短轮询策略，构建日志使用 SSE `logs/stream` 实时推送，发布日志在发布运行中按同一短轮询间隔刷新。日志 stream 以已落库 `BuildLog.content` 的字节 offset 作为 event id，前端可通过 `after` 或 `Last-Event-ID` 在刷新/重连后从指定 offset 继续读取；普通 GET 日志接口保留为历史查看兜底。
- 项目空间级 Hook 配置支持 `preBuild`、`postBuild`、`preDeployment`、`postDeployment` 四个阶段。Hook 配置本身不再维护启停状态；构建阶段 Hook 是否执行由模块配置的“启用构建钩子”开关决定，同阶段多个 Hook 通过拖拽排序写入 `runOrder`。Hook 脚本不做模板渲染，统一通过环境变量读取平台上下文和项目变量/密钥；平台保留镜像 tag 等配置字段的模板渲染能力。`preBuild` 和 `postBuild` 随 BuildJob 下发给 Builder executor，在源码 checkout 后分别于镜像构建前后执行，并注入 `LITEYUKI_BUILD_RUN_ID`、`LITEYUKI_GIT_SHA`、`LITEYUKI_GIT_SHORT_SHA`、`LITEYUKI_IMAGE_REF` 等变量；`preDeployment` 和 `postDeployment` 由 Worker 在目标项目 namespace 中创建临时 Kubernetes Job 执行，使用本次发布镜像、同一套 ConfigMap/Secret 和 `LITEYUKI_RELEASE_ID`、`LITEYUKI_DEPLOYMENT_TARGET_ID` 等上下文环境变量。每次运行创建 `HookRun` 脚本快照和 `HookRunLog`，支持超时和 `fail/ignore` 失败策略；`preDeployment` 失败默认阻断发布，避免数据库迁移失败后继续 rollout。
- 构建触发入口必须选择应用模块配置；应用创建和应用信息配置不再自动生成或填写模块配置，模块配置本身不再维护默认属性。部署配置在模块配置编辑入口中维护，用 `DeploymentTarget` 绑定 `moduleId -> environmentId` 并配置自动部署策略。新增模块时可先在模块表单内添加部署配置草稿，但这些草稿必须等模块保存成功、获得稳定 `moduleId` 后再一起创建，避免模块尚未落库时产生孤儿部署配置。模块配置表单还可以可选配置访问，按当前应用和环境创建或更新 `GatewayRoute`，留空域名时继续使用平台默认域名生成规则。手动发布 Release 时会从选中 BuildRun 继承 `moduleId`；应用部署页以 DeploymentTarget 为主列表展示部署配置、最新发布状态、部署日志、回滚和手动发布入口，让配置归属和运行态结果能互相对齐。
- executor 镜像需包含 git、buildctl、shell 和基础诊断工具。
- executor 使用 BuildKit rootless 的 `buildctl-daemonless.sh` 构建并推送镜像。
- executor 在 Builder 启用缓存时使用 BuildKit registry cache；缓存不存在或首次构建时按 cache miss 继续构建，不阻塞主流程。
- Docker executor 容器名统一使用 `liteyuki-devops-buildtask_{buildid}`，其中 `buildid` 对应平台 BuildJob ID，便于本地排查、日志关联和残留容器清理。Builder 续租时上报 executor 容器名；执行结束后由 Builder 主动清理容器，若运行中的 executor 容器被外部删除，Builder 回写 `executor_lost`，Worker 的 lease reconciler 兜底收敛为 `lost`。
- executor 不持有平台 API token，不直接回调 API；Builder Agent 只持有受限 Builder token。
...
- Builder Agent 读取 `result.json`，回写 imageRef、imageDigest、sourceCommit、Git commit author 和最终状态；平台触发人由 API 创建 BuildRun 时保存。
...
- 支持构建日志查看。
- 支持一个应用多个模块配置，并在模块配置中通过部署配置把模块配置绑定到环境；模块配置表单可选维护该应用在对应环境的访问入口。
...
- 项目空间内部应用、环境、构建变量和密钥和成员必须整合到工作台 `ContentTabs` 中，旧的 `/apps`、`/repositories`、`/members` 列表入口只做兼容重定向，不作为主要可见入口。
- 构建变量和密钥是项目空间级资源，由项目工作台集中创建、编辑和删除；应用配置页不再提供变量和密钥选择入口，构建时默认注入项目空间启用的变量和密钥。
