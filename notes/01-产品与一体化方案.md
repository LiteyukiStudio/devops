...
- 平台自身只发布容器镜像到 DockerHub `liteyukistudio` 命名空间，仅构建 `linux/amd64` 和 `linux/arm64`，镜像为 `devops-api`、`devops-worker`；`v*` tag 同步发布版本 tag 和 `latest`，分支构建发布 `latest`，不额外构建或上传 GitHub Release 二进制产物。
...
- 开发 Compose 不读取宿主机 `.env.development`；`.env.development` 只给宿主机 `go run` 进程使用，`.env.worker` 面向 worker 容器，`.env.builder` 面向 builder 容器。容器内数据库、Redis 和 API 地址使用容器语义配置，避免宿主机开发配置里的 `localhost` 地址在容器内失效。
