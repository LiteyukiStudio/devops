...
- 第一版只展示和管理由 Luna DevOps 创建或显式打上平台标签的资源，识别依据优先使用 Kubernetes labels/annotations，例如 `app.kubernetes.io/managed-by=liteyuki-devops`、`liteyuki.devops/project-id`、`liteyuki.devops/application-id`、`liteyuki.devops/environment-id`、`liteyuki.devops/release-id`。
